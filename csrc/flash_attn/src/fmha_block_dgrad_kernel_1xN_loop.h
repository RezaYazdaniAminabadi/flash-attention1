...
template <typename Smem_dp_sum, int M, typename elem_type=__half>
...
inline __device__ void compute_block_dq_dk_dv_1xN_one_iter(const Params &params, Prng &ph,
    ...
    // if (true) {
    if (Is_first || mask_val % 2 == 1) {
        dot_do_o<Smem_dp_sum, Gmem_tile_do::LDGS, elem_type>(dp_sum_regs, gmem_do.fetch_, gmem_o.fetch_, smem_dp_sum, 0);
    ...
    for( int l = 0; l < steps; l++ ) {
        ...
        if (!Is_dropout) {
            ...
            for( int mi = 0; mi < Mma_tile_p::MMAS_M; mi++ ) {
                #pragma unroll
                for( int ni = 0; ni < Mma_tile_p::MMAS_N; ni++ ) {
                    frag_p[mi][ni].hmul(frag_dp[mi][ni], !is_fp16_type);
                }
            }
        } else {
        ...
        if (not_last_iter) {
            ...
            if (Is_first || mask_val_next % 2 == 1) {
                ...
                dot_do_o<Smem_dp_sum, Gmem_tile_do::LDGS, elem_type>(dp_sum_regs, gmem_do.fetch_, gmem_o.fetch_, smem_dp_sum, (l + 1) % 2);
        ...
        if (is_final_write) {
            ...
            // Output the values.
            gmem_dq_tmp.template store<elem_type>(dq_out, 0);
        }
